// Converteert DMS-coördinaten naar decimaal en genereert SQL insert statements
...
try {
  ...
  const match = env.match(/NEXT_PUBLIC_GOOGLE_MAPS_KEY=(.+)/);
  if (match) MAPS_KEY = match[1].trim();
} catch {}
if (!MAPS_KEY) { console.error('Waarschuwing: geen NEXT_PUBLIC_GOOGLE_MAPS_KEY gevonden'); }
...
for (const block of blocks) {
  ...
  const parts = coordLine.split(/\s+(?=\d+°)/);
  if (parts.length !== 2) continue;

  const lat = dmsToDecimal(parts[0]);
  const lng = dmsToDecimal(parts[1]);

  // Zorg voor unieke slugs
  const baseSlug = toSlug(address);
  slugCounts[baseSlug] = (slugCounts[baseSlug] || 0) + 1;
  const slug = slugCounts[baseSlug] > 1 ? `${baseSlug}-${slugCounts[baseSlug]}` : baseSlug;

  locations.push({ name: toTitleCase(address), slug, lat, lng, address });
}

// Genereer SQL
let sql = `-- ============================================================
-- SEED DATA: bollenvelden Bollenstreek (${locations.length} locaties)
-- Gegenereerd uit Tulipday Adressen lijst van Tom de Vroomen
-- ============================================================
...
const rows = locations.map((loc, i) => {
  ...
  const streetViewUrl = MAPS_KEY
    ? `https://maps.googleapis.com/maps/api/streetview?size=800x500&location=${loc.lat},${loc.lng}&fov=90&pitch=5&key=${MAPS_KEY}`
    : null;
  const imgSql = streetViewUrl ? `'${streetViewUrl}'` : 'null';
  return `(
  gen_random_uuid(),
  '${loc.name.replace(/'/g, "''")}',
  '${loc.slug}',
  ...
  'Bollenveld langs de ${toTitleCase(loc.address.replace(/\d+$/, '').trim()).replace(/'/g, "''")} in de Bollenstreek.',
  'Bollenveld in de Bollenstreek, onderdeel van de Tulipday Adressen collectie. Bereikbaar langs de weg, ideaal voor een fietstocht of autorit door de regio.',
  'tulip',
  'blooming',
  4, 4,
  'roadside_only',
  'Beperkt parkeren langs de kant van de weg. Fiets aanbevolen.',
  'Vroeg in de ochtend of golden hour voor de mooiste foto''s.',
  ${imgSql},
  false, true
)${comma}`;
});
...
console.log(sql);
console.error(`✓ ${locations.length} locaties verwerkt`);
